	...
	public static void biomeLoadingEvent(final BiomeLoadingEvent event) {
		...
		if (name != null) {
			...
			if (DoTBConfig.MULBERRY_GENERATION.get().contains(name.toString())) {
				generation.addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, cropsFeature.configured(new BlockClusterFeatureConfig.Builder(new SimpleBlockStateProvider(DoTBBlocksRegistry.MULBERRY.get().defaultBlockState()), new DoTBBlockPlacer()).tries(DoTBConfig.MULBERRY_ROLLS.get()).xspread(2).zspread(2).build()).decorated(Features.Placements.ADD_32).decorated(Features.Placements.HEIGHTMAP_SQUARE));
			}
			if (DoTBConfig.RICE_GENERATION.get().contains(name.toString())) {
				generation.addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, cropsFeature.configured(new BlockClusterFeatureConfig.Builder(new SimpleBlockStateProvider(DoTBBlocksRegistry.RICE.get().defaultBlockState()), new DoTBBlockPlacer()).tries(DoTBConfig.RICE_ROLLS.get()).build()).decorated(Features.Placements.HEIGHTMAP_DOUBLE_SQUARE));
			}
			if (DoTBConfig.WILD_GRAPE_GENERATION.get().contains(name.toString())) {
				generation.addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, cropsFeature.configured(new BlockClusterFeatureConfig.Builder(new SimpleBlockStateProvider(DoTBBlocksRegistry.WILD_GRAPE.get().defaultBlockState()), SimpleBlockPlacer.INSTANCE).tries(DoTBConfig.WILD_GRAPE_ROLLS.get()).xspread(2).zspread(2).build()).decorated(Features.Placements.ADD_32).decorated(Features.Placements.HEIGHTMAP_SQUARE));
			}
			if (DoTBConfig.WILD_MAIZE_GENERATION.get().contains(name.toString())) {
				generation.addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, cropsFeature.configured(new BlockClusterFeatureConfig.Builder(new SimpleBlockStateProvider(DoTBBlocksRegistry.WILD_MAIZE.get().defaultBlockState()), new DoTBBlockPlacer()).tries(DoTBConfig.WILD_MAIZE_ROLLS.get()).xspread(2).zspread(2).build()).decorated(Features.Placements.ADD_32).decorated(Features.Placements.HEIGHTMAP_SQUARE));
			}
			if (DoTBConfig.GERANIUM_PINK_GENERATION.get().contains(name.toString())) {
				generation.addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, cropsFeature.configured(new BlockClusterFeatureConfig.Builder(new SimpleBlockStateProvider(DoTBBlocksRegistry.GERANIUM_PINK.get().defaultBlockState()), SimpleBlockPlacer.INSTANCE).tries(DoTBConfig.GERANIUM_PINK_ROLLS.get()).xspread(2).zspread(2).build()).decorated(Features.Placements.HEIGHTMAP_SQUARE));
			}
		}
	}
}