...
public class LatticeWaxedOakWindowBlock extends BlockDoTB {
	...
	public LatticeWaxedOakWindowBlock() {
		super("lattice_waxed_oak_window", Material.GLASS, 1.0F, 1.0F);
	    this.setBurnable();
    ...
    private boolean canConnectVertical(World worldIn, BlockPos pos) {
    	if(worldIn.getBlockState(pos.down()).getBlock() instanceof LatticeWaxedOakWindowBlock){
			return !(worldIn.getBlockState(pos.up()).getBlock() instanceof LatticeWaxedOakWindowBlock);
    	}
    	return false;
    }
    ...
    private boolean canConnectHorizontal(BlockState state, World worldIn, BlockPos pos) {
    	...
    	if(worldIn.getBlockState(pos).getBlock() instanceof LatticeWaxedOakWindowBlock){
