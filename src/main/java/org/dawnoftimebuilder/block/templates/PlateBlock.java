...
public class PlateBlock extends WaterloggedBlock
{
	...
	{
		super(properties);
		this.registerDefaultState(this.defaultBlockState().setValue(PlateBlock.FACING, Direction.NORTH)
	...
	{
		final BlockState state = super.getStateForPlacement(context).setValue(PlateBlock.FACING,
				context.getHorizontalDirection());
		return state.setValue(PlateBlock.SHAPE,
				PlateBlock.getShapeProperty(state, context.getLevel(), context.getClickedPos()));
	}
			...
			final IWorld worldIn, final BlockPos currentPos, final BlockPos facingPos)
	{
		...
		return facing.getAxis().isHorizontal()
				? stateIn.setValue(PlateBlock.SHAPE, PlateBlock.getShapeProperty(stateIn, worldIn, currentPos))
				: stateIn;
	}
	...
	private static StairsShape getShapeProperty(final BlockState state, final IBlockReader worldIn, final BlockPos pos)
	{
		...
		if (PlateBlock.isBlockPlate(adjacentState))
		{
			...
			if (adjacentDirection.getAxis() != state.getValue(PlateBlock.FACING).getAxis()
					&& PlateBlock.isDifferentPlate(state, worldIn, pos, adjacentDirection.getOpposite()))
			{
		...
		if (PlateBlock.isBlockPlate(adjacentState))
		{
			final Direction adjacentDirection = adjacentState.getValue(PlateBlock.FACING);
			if (adjacentDirection.getAxis() != state.getValue(PlateBlock.FACING).getAxis()
					&& PlateBlock.isDifferentPlate(state, worldIn, pos, adjacentDirection))
			{
	...
	private static boolean isDifferentPlate(final BlockState state, final IBlockReader worldIn, final BlockPos pos,
			final Direction face)
	{
		final BlockState adjacentState = worldIn.getBlockState(pos.relative(face));
		return !PlateBlock.isBlockPlate(adjacentState)
				|| adjacentState.getValue(PlateBlock.FACING) != state.getValue(PlateBlock.FACING);
	}
		...
		switch (mirrorIn)
		{
			case LEFT_RIGHT:
				if (direction.getAxis() == Direction.Axis.Z)
				{
					...
					{
						case INNER_LEFT:
							return state.rotate(Rotation.CLOCKWISE_180).setValue(PlateBlock.SHAPE,
									StairsShape.INNER_RIGHT);
						case INNER_RIGHT:
							return state.rotate(Rotation.CLOCKWISE_180).setValue(PlateBlock.SHAPE,
