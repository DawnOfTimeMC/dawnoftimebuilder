...
public class TileEntityDryerRenderer extends TileEntitySpecialRenderer<DoTBTileEntityDryer> {

	@Override
	public void render(DoTBTileEntityDryer tileEntity, double x, double y, double z, float partialTicks, int destroyStage, float alpha){
		/*
		ItemStack itemStack;
		double xStart = 0.0D;
		double yStart = 0.0D;
		...
		if(tileEntity.getBlockType() instanceof IBlockDisplayer){
			IBlockDisplayer blocks = (IBlockDisplayer) tileEntity.getBlockType();
			int meta = tileEntity.getBlockMetadata();
			xStart = blocks.getDisplayerX(meta);
			yStart = blocks.getDisplayerY(meta);
			zStart = blocks.getDisplayerZ(meta);
		}
		...
		for(int i = 0; i < 9; i++){
			...
			if (!itemStack.isEmpty()) {
				...
				if(item instanceof ItemBlock){
					...
					if(blocks instanceof IBlockSpecialDisplay){
						float scale = ((IBlockSpecialDisplay)blocks).getDisplayScale(tileEntity.getBlockMetadata());
