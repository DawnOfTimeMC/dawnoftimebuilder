...
public class BlockRedSculptedPlasteredStoneFrieze extends DoTBBlock {
    ...
    private static final AxisAlignedBB AABB_SPIKE_NORTH = new AxisAlignedBB(0.21875D, 0.25D, 0.25D, 0.78125D, 0.8125D, 0.4375D);
    private static final AxisAlignedBB AABB_SPIKE_NW = new AxisAlignedBB(0.25D, 0.25D, 0.25D, 0.71875D, 0.8125D, 0.71875D);
    
    public BlockRedSculptedPlasteredStoneFrieze() {
    	super("red_sculpted_plastered_stone_frieze", Material.ROCK, 1.5F, SoundType.STONE);
    ...
    private static List<AxisAlignedBB> getCollisionBoxList(IBlockState bstate) {
        ...
        EnumsBlock.EnumStairsShape shape = bstate.getValue(SHAPE);
        switch (shape) {
        	default:
            case STRAIGHT:
    ...
    public IBlockState getActualState(IBlockState state, IBlockAccess worldIn, BlockPos pos) {
    	return state.withProperty(SHAPE, getRedStoneFriezeShape(state, worldIn, pos));
    }
    ...
    private static EnumsBlock.EnumStairsShape getRedStoneFriezeShape(IBlockState state, IBlockAccess worldIn, BlockPos pos) {
        ...
        if (isBlockRedStoneFrieze(stateAdjacent)) {
            EnumFacing facingAdjacent = stateAdjacent.getValue(FACING);
            if (facingAdjacent.getAxis() != (state.getValue(FACING)).getAxis() && isDifferentRedStoneFrieze(state, worldIn, pos, facingAdjacent.getOpposite())) {
        ...
        stateAdjacent = worldIn.getBlockState(pos.offset(facing.getOpposite()));
        if (isBlockRedStoneFrieze(stateAdjacent)) {
            EnumFacing facingAdjacent = stateAdjacent.getValue(FACING);
            if (facingAdjacent.getAxis() != (state.getValue(FACING)).getAxis() && isDifferentRedStoneFrieze(state, worldIn, pos, facingAdjacent)) {
                if (facingAdjacent == facing.rotateYCCW()) return EnumsBlock.EnumStairsShape.INNER_LEFT;
    ...
    private static boolean isDifferentRedStoneFrieze(IBlockState p_185704_0_, IBlockAccess p_185704_1_, BlockPos p_185704_2_, EnumFacing p_185704_3_) {
        ...
        return !isBlockRedStoneFrieze(iblockstate) || iblockstate.getValue(FACING) != p_185704_0_.getValue(FACING);
    }
    
    private static boolean isBlockRedStoneFrieze(IBlockState state) {
        return state.getBlock() instanceof BlockRedSculptedPlasteredStoneFrieze;
    }
