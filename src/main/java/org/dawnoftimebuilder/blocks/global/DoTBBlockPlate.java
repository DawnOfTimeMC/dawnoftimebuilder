...
public class DoTBBlockPlate extends DoTBBlock {
	...
	protected boolean canConnectWithDoTPlate = true;
	...
	public DoTBBlockPlate(String name, Material materialIn, float hardness, SoundType sound) {
		...
		this.setDefaultState(this.blockState.getBaseState().withProperty(FACING, EnumFacing.NORTH).withProperty(SHAPE, DoTBBlockPlate.EnumShape.STRAIGHT));
	...
	private static List<AxisAlignedBB> getCollisionBoxList(IBlockState state) {
		...
		DoTBBlockPlate.EnumShape shape = state.getValue(SHAPE);
		switch (shape) {
			default:
			case STRAIGHT:
	...
	public BlockFaceShape getBlockFaceShape(IBlockAccess worldIn, IBlockState state, BlockPos pos, EnumFacing facing) {
		...
		if (shape != DoTBBlockPlate.EnumShape.OUTER_LEFT && shape != DoTBBlockPlate.EnumShape.OUTER_RIGHT) {
	...
	private static DoTBBlockPlate.EnumShape getVerticalSlabShape(IBlockState state, IBlockAccess worldIn, BlockPos pos) {
		EnumFacing facing = state.getValue(FACING);
		...
		if (stateAdjacent.getBlock() instanceof DoTBBlockPlate) {
			...
			if (facingAdjacent.getAxis() != state.getValue(FACING).getAxis() && block.isNotConnectible(state, worldIn, pos, facingAdjacent.getOpposite())) {
				if (facingAdjacent == facing.rotateYCCW()) return DoTBBlockPlate.EnumShape.OUTER_LEFT;
				else return DoTBBlockPlate.EnumShape.OUTER_RIGHT;
			}
		}
		...
		if (stateOpposite.getBlock() instanceof DoTBBlockPlate) {
			...
			if (facingOpposite.getAxis() != state.getValue(FACING).getAxis() && block.isNotConnectible(state, worldIn, pos, facingOpposite)) {
				if (facingOpposite == facing.rotateYCCW()) return DoTBBlockPlate.EnumShape.INNER_LEFT;
				else return DoTBBlockPlate.EnumShape.INNER_RIGHT;
			}
		}

		return DoTBBlockPlate.EnumShape.STRAIGHT;
	}
	...
	private static boolean isConnectibleWithDoTPlate(IBlockState state) {
		if(state.getBlock() instanceof DoTBBlockPlate){
			return ((DoTBBlockPlate) state.getBlock()).canConnectWithDoTPlate;
		}else return false;
	}
	...
	{
		return this.getDefaultState().withProperty(FACING, placer.getHorizontalFacing()).withProperty(SHAPE, DoTBBlockPlate.EnumShape.STRAIGHT);
	}
		...
		switch (mirrorIn) {
			case LEFT_RIGHT:
				if (enumfacing.getAxis() == EnumFacing.Axis.Z) {
					switch (shape) {
						case OUTER_LEFT:
							return state.withRotation(Rotation.CLOCKWISE_180).withProperty(SHAPE, DoTBBlockPlate.EnumShape.OUTER_RIGHT);
						case OUTER_RIGHT:
							return state.withRotation(Rotation.CLOCKWISE_180).withProperty(SHAPE, DoTBBlockPlate.EnumShape.OUTER_LEFT);
						case INNER_RIGHT:
							return state.withRotation(Rotation.CLOCKWISE_180).withProperty(SHAPE, DoTBBlockPlate.EnumShape.INNER_LEFT);
						case INNER_LEFT:
							return state.withRotation(Rotation.CLOCKWISE_180).withProperty(SHAPE, DoTBBlockPlate.EnumShape.INNER_RIGHT);
						default:
